...
// Extended to add battery & BT information  
...
static uint32_t delta = 60000;  // 60000=once per minute 1000=once per second
...
static void directAccess(GContext *ctx)
{
  ...
  int i;
  uint8_t xdp=(currentMinute<30)?2:XREZ-6;  // extended data position
  if(mag(tx)>mag(ty))
  {
    ty/=mag(tx)>>4;
    tx/=mag(tx)>>4; 
  ...
  // draw the extended data
  
  //btconnected=true;
  //batteryLevel=50;
  
  uint8_t ydp=YREZ-6;
  int col=(btconnected)?0x0055ff:0xff0000;
  for(i=0;i<10;i++)
  {
    box_c(bits,col,xdp,ydp,4,2,2);
    if(i*10>batteryLevel) box_c(bits,0x000000,xdp+1,ydp,2,2,1);
    ydp-=2;
  }
